...
### Fixed
- `src/db.js`, `src/sql/mempool_transactions.sql` — mempool ingestion no longer pre-allocates rows in the replicated `index_addresses` / `index_transactions` lookup tables. `insertMempoolTransaction` previously called `createTransaction(tx.hash)` / `createAddress(tx.source)` / `createAddress(tx.destination)` to obtain integer ids before a transaction was confirmed, so observing a transaction in the mempool minted new lookup-table rows. Those lookup tables are part of the replicated decoder set, but mempool arrival order is node-local and non-deterministic — two nodes therefore assigned **different** AUTO_INCREMENT ids to the same address / transaction hash purely based on which they happened to see first. A follower that took a full snapshot of its lookup tables from one source and then streamed live block data from another would silently mix two incompatible id namespaces, resolving `transactions.source_id` / `destination_id` to the wrong address, with no hash-rejection guard to catch the skew. `mempool_transactions` now stores the raw `tx_hash` / `source` / `destination` strings directly (it is intentionally excluded from replication, so it never needed lookup ids), and the `deleteAndCompareTxsNotInList` reconciliation reads those strings without a join. Lookup ids are now allocated **only** during deterministic block-confirmation processing (`insertTransaction`), where every node following the same chain from the same genesis assigns identical ids regardless of mempool history or sync source. Schema change delivered as the new `src/sql/migrations/2026-06-15-mempool-raw-strings.sql` (mode=manual: drops and recreates the transient table, which is rebuilt from the live mempool within one poll interval). New unit and e2e regression coverage asserts mempool ingestion never touches the lookup tables and stores raw strings verbatim.
- `src/XChainDecoder.js` — the decoder now accepts the five documented short-form ACTION-name aliases (`TRANSFER`→`SEND`, `ADDR`→`ADDRESS`, `DROP`→`AIRDROP`, `CAST`→`BROADCAST`, `MSG`→`MESSAGE`) instead of silently dropping them. The `VALID_ACTION_NAMES` allowlist holds only canonical names, so any on-chain transaction whose leading token was an alias failed the block-loop name gate (`unknown ACTION name`) and was never written to the database — the alias feature was completely non-functional, and a spec-following client encoding e.g. `TRANSFER|…` (documented as BRC20/SRC20-compatible) lost every transaction with no error surfaced to the submitter. A new static `ACTION_ALIASES` map is consulted immediately after the action name is split from the payload and *before* the allowlist gate; when an alias matches, the canonical name is substituted back into the stored payload so the database holds only canonical names regardless of which spelling appeared on-chain (no aliased-vs-canonical row drift, and downstream name handling needs no per-alias special-casing). The same normalise-then-gate is now applied on the mempool-insertion path, which previously had *no* ACTION-name validation at all: alias-named (and otherwise unknown-named) transactions entered `mempool_transactions` as pending and then silently vanished at confirmation. The mempool rewrite splices only the leading name bytes at the first `|`, preserving any binary payload tail (e.g. gated-FILE / MESSAGE v2 binary) byte-for-byte. **Operational note:** historical blocks that contained alias-named transactions were never written to the decoder DB; a targeted rescan from the earliest affected height is required to backfill them (`TRANSFER` is the highest priority to recover given its BRC20/SRC20 documentation). Existing unit and security suites continue to pass.
- `src/XChainDecoder.js`, `src/db.js` — the per-block parse loop no longer issues one dispenser-lookup database round-trip per transaction output. `parseTransaction` previously called `db.isThereADispenserForAddress(outputAddress)` (a `SELECT COUNT(*)` join) once for every output of every transaction; at mainnet block sizes (thousands of txs, ~2.5 outputs each) that was 5,000–10,000 serialized queries per block in the hottest path, enough for the decoder to fall behind the chain tip and stall every downstream consumer. The set of open-dispenser addresses is now loaded once per block via the new `db.getAllOpenDispenserAddresses()` (a single query, run after expired dispensers are pruned) into a `Set`, and each output is tested in memory with `Set.has()`. The set is kept current within the block by adding any dispenser opened by an earlier transaction, so a same-block open-then-pay is still recognized as a dispense — matching the previous per-output lookup's timing exactly (a dispenser is only visible after its opening transaction is processed). New unit tests assert the DB layer returns a `Set` from a single query (NULL-address rows skipped), that `parseTransaction` issues zero per-output dispenser lookups, and that dispense detection is driven solely by the passed set.
- `src/CryptoNetworks.js` — corrected the Litecoin dust threshold from `546` to `5460` litoshis for all three `litecoin-*` networks, matching Litecoin Core's relay policy (dust relay fee 10× Bitcoin's). The decoder does not currently consume `dustThreshold`, so this is a consistency fix keeping the network table aligned with the platform-wide value now used by the encoder and SDK. (Restores the 1.11.6 fix, which was later reverted to match the encoder's then-incorrect 546 — all platform copies now carry 5460 together.)
