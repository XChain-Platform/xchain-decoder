...
### Changed
- Test suites synchronize on the decoder's actual progress signal through a single shared `waitUntil(predicate, timeout)` poll helper (`test/helpers/waitUntil.js`) instead of fixed `setTimeout` sleeps, removing a class of timing-dependent flakes across the chaos, integration, and end-to-end tiers.
