...
### Fixed
- `src/db.js` — `dropDatabase()` (the test-only helper that resets database state between runs) now also drops `transaction_outputs`, `dispensers`, `mempool_transactions`, and `pubkeys`. It previously dropped only `blocks`, `transactions`, `index_addresses`, `index_transactions`, and `events`, leaving those four tables intact; a suite that called the helper to reset state accumulated stale rows across re-runs against the same database, producing non-deterministic failures in dispenser-lookup and pubkey-dedup assertions. The four new drops run first so `pubkeys` is removed before `index_addresses`, which it references via foreign key (a reversed order would fail the DROP with a constraint error).
