 ...
 * the sibling. Several modules in these services are exactly that shape. A file with no
 ...
 * shape a restructure here deletes outright.
 *
 ...
 * option: the twin-copier script alone byte-copies src/ files outward between
 * repos and no CI job runs it, so a module held only from there
 ...
 */
// Empty here, and that is a measured claim rather than an oversight. Every require
// under this repo's src/ is a literal: swept for the four computed forms
// (`require('./' +`, a template literal, `require(path.join(`, and a variable
// joined onto a 'src/...' literal) the whole tree returns nothing under src/.
// The computed requires this repo does have all live under test/, where they
// load benchmark scenarios and cross-repo fixtures rather than src/ modules, so
// none of them holds a src/ file alive. Add a row here the moment src/ grows
// one, or the file it loads starts reading unreachable.
const DYNAMIC_EDGES = [];
