...
#
# Run the E2E tier against a throwaway containerised venue.
#
# Brings up test/e2e/fixtures/docker-compose.test.yml (a regtest bitcoind and a
# MariaDB, both on tmpfs), runs mocha against it, then tears the venue down
...
teardown() {
  if [ "$KEEP_VENUE" = "1" ]; then
    echo "[e2e] KEEP_VENUE=1, leaving the venue up ($COMPOSE_FILE)"
    return
  fi
...
if ! command -v docker >/dev/null 2>&1; then
  echo "[e2e] docker is required: this tier runs against a containerised regtest node." >&2
  exit 1
fi
