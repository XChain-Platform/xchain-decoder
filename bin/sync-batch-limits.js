 ...
 *
 *   node bin/sync-batch-limits.js          # rewrite the vendored module
 ...
 * in batch_limits_vendoring.test.js instead.
 *
 ...
 * requires deriveFromSibling() and renderModule() from HERE, so the check and the fix can
 * never implement two different ideas of what the vendored file should say.
 *
 ********************************************************************/
...
function main(){
    ...
    if (check){
        ...
        console.error('run: node bin/sync-batch-limits.js');
        process.exit(1);
    }
