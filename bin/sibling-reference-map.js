 ...
 *
 *   text       any literal run of `xchain-decoder/src/<path>` in any text file:
 *              a relative require, a comment, a shell script, a markdown runbook.
 *   join       a path built segment by segment, the shape
 *              path.join(root, 'xchain-decoder', 'src', 'foo.js'). When every
 ...
 *              `const R = path.resolve(__dirname, '../../xchain-decoder')` then
 *              path.join(R, 'src', 'foo.js'), path.join(R, 'src/foo.js') or
 *              `${R}/src/foo.js`. The root may equally be
 *              process.env.XCHAIN_DECODER_PATH, a candidate array filtered to its
 ...
 *              this checkout, `decoderFile('src/rollback.js')`. A call over a
 *              literal array, `decoderFile('src/' + twin)`, is emitted once per
 ...
 *              `copy_twin xchain-decoder "src/$f"`, resolved against the literal
...
const REPO_NAME = 'xchain-decoder';
...
// `xchain-decoder/src/<path>`, however it was spelled: a relative require
// (`../../xchain-decoder/src/utility.js`), a prose mention, a shell path.
const TEXT_REFERENCE = new RegExp(`${NAME_RE}\\/(src\\/[A-Za-z0-9_@.\\-/]+)`, 'g');

// path.join(..., 'xchain-decoder', 'src', ...): the tail is captured raw and
...
// Environment variables that hold a checkout of this repo. XCHAIN_DECODER_SQL_PATH
...
const ENV_ROOT_VARS = ['XCHAIN_DECODER_PATH', 'XCHAIN_DECODER_DIR'];
...
function rootPrefix(init) {
    ...
    while ((m = re.exec(init)) !== null) {
        ...
        // path.join(base, 'xchain-decoder', 'src', 'sql').
...
function collectRootProducers(text) {
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        // `const p = decoderFile('src/x.js')` look like another checkout.
 ...
 * path per element: the explorer's hub-mirror guard and a sibling's post-move twin
 ...
 * checkout, the `decoderFile('src/rollback.js')` idiom. Recognised by a body
 ...
 * array of rows, which is how both SHARED_GATES and a sibling's post-move twin
...
function helperReferences(text, helpers, lists, strings) {
    ...
    for (const [name, suffix] of helpers) {
        ...
        let m;
        while ((m = re.exec(text)) !== null) {
            // `function decoderFile(rel){` reads as a call of itself. Its
 ...
 * relative path, `copy_twin xchain-decoder "src/$f"`, where the file comes from
