...
// this repo's then-declared siblings ran 1063 passing / 30 pending where the
// full checkout runs 1082 / 11, so 19 guards had silently not run.
//
...
const SIBLINGS = [
    { repo: 'xchain-encoder', envs: ['XCHAIN_ENCODER_DIR'],
      marker: 'src',
      guards: 'compiled push-size and roundtrip conformance, plus taproot-envelope validator parity' },
    { repo: 'xchain-documentation', envs: ['XCHAIN_DOCS_DIR', 'XCHAIN_DOCUMENTATION_DIR'],
      marker: 'protocol',
      guards: 'action-manifest conformance, the canonical ORACLE_FEE_OUTPUT_ACTIVATION map, and the taproot golden vectors' },
