...
// Regression guard: the BigInt-safe bufferutils patch must be active in-process
// (not only via the Dockerfile COPY over node_modules), or a Dogecoin output
// > 2^53-1 sat (~90.07M DOGE) throws during block decode and wedges the decoder
...
describe('applyBufferutilsPatch', function () {
    ...
    it('patches the shared bitcoinjs-lib bufferutils module in place', function () {
        assert.strictEqual(bufferutils, require('bitcoinjs-lib/src/bufferutils'))
        assert.strictEqual(bigIntBufferutilsActive(), true)
    })
    ...
    it('BufferReader.readUInt64 returns values above 2^53-1 as unsigned BigInt', function () {
        ...
        assert.strictEqual(at2pow53.readUInt64(), 9007199254740992n)
        ...
        assert.strictEqual(highBit.readUInt64(), 9223372036854775808n)

        const max = new bufferutils.BufferReader(Buffer.from('ffffffffffffffff', 'hex'))
        assert.strictEqual(max.readUInt64(), 18446744073709551615n)
    })
    ...
    it('BufferWriter.writeUInt64 round-trips a BigInt value', function () {
        ...
        assert.strictEqual(new bufferutils.BufferReader(writer.end()).readUInt64(), 9007199254740993n)
    })

    it('exported readUInt64LE/writeUInt64LE keep the Number-safe contract', function () {
        const buf = Buffer.alloc(8)
        assert.strictEqual(bufferutils.writeUInt64LE(buf, Number.MAX_SAFE_INTEGER, 0), 8)
        assert.strictEqual(bufferutils.readUInt64LE(buf, 0), Number.MAX_SAFE_INTEGER)
        assert.throws(() => bufferutils.readUInt64LE(Buffer.from('0000000000002000', 'hex'), 0),
            /value out of range/)
    })
    ...
    it('decodes a transaction whose output value exceeds 2^53-1 sat', function () {
        ...
        const tx = decoder.transactionFromHex(LARGE_OUTPUT_TX_HEX)
        assert.strictEqual(BigInt(tx.outs[0].value), 9007199254740992n)
    })
})
